...
def add_tracks(data,key,new_urls,conf_urls):
    ...
    for url in new_urls:
        ...
        if id not in data[key][u'tracks'].keys():
            ...
            data[key][u'tracks'][id][u'title'] = u''
            data[key][u'tracks'][id][u'description'] = u''
            data[key][u'tracks'][id][u'speaker'] = u''
            data[key][u'tracks'][id][u'speaker'] = u''
            data[key][u'tracks'][id][u'url'] = url
            data[key][u'tracks'][id][u'url_slides'] = u''
            data[key][u'tracks'][id][u'url_video'] = u''
...
def update_tracks(data,key):
    ...
    for i,id in enumerate(data[key][u'tracks']):
        ...
        if u'url_slides' in track.keys() and track[u'url_slides'] == u'' or u'url_slides' not in track.keys():
            ...
            title,description,url_slides,url_video,speaker,about_speaker = extract_track_data(track[u'url'])

            if url_slides != u'' and not TEXT_ONLY:
                # Slides were found
                print(u'Slide url found for %s, will try to download' % id)
                download_slides(url_slides)

            # Save data for this track
            track[u'title'],track[u'description'],track[u'url_slides'],track[u'url_video'],track[u'speaker'],track[u'about_speaker'] = title,description,url_slides,url_video,speaker,about_speaker

        elif RE_DOWNLOAD and u'url_slides' in track.keys() and track[u'url_slides'] != u'':
            # Re-download slides
            print(u'Redownloading slides %s' % id)
            download_slides(track[u'url_slides'])
    ...
    if r_track.status_code != 200:
        ...
        for line in content.split(u'\n'):
            ...
            if step == 0 and u'<main class="event__info">' in line:
                ...
                step = 3
            elif step == 3 and '<div class="event__downloads">' in line:
                debug(u'Found download section')
                step = 4
            elif step == 4 and u'Download Slides' in line:
                debug(u'Found slides url')
                url_slides = line.split(u'"')[1]
            elif step == 4 and u'Watch Video' in line:
                debug(u'Found video url')
                url_video = line.split(u'"')[1]
            elif step == 4 and '</div>' in line:
                debug(u'End of download section')
                step = 5
            elif step == 5 and u'<div>' in line:
                debug(u'Found description section')
                step = 6
            elif step == 6 and '</div>' in line:
                debug(u'End of description section')
                step = 7
            elif step == 6:
                debug(u'Found new line in description')
                description += line + u'\n'
            elif step == 7 and u'<div class="event__speakers">' in line:
                debug(u'Found speaker section')
                step = 8
            elif step == 8 and u'<span class="icon-label">' in line:
                speaker = line.split(u'>')[1].split(u'<')[0]
                debug(u'Found speaker')
            elif step == 8 and u'<article class="bio">' in line:
                debug(u'Found speaker bio')
                step = 9
            elif step == 9 and u'</article>' in line:
                debug(u'End of bio section')
                step = 10
            elif step == 10 and u'</div>' in line:
                debug(u'End of speaker section')
                break
    return title,description,url_slides,url_video,speaker,about_speaker
